  ...
  <div>
    <div class="panel">
      <!-- 使用 Element Plus 表格组件 -->
       ...
       <!-- 单元格合并方法 objectSpanMethod -->
       <!-- 表头样式header-cell-style -->
        <!-- 单元格样式回调 tableCellStyle -->

      <el-table
        :data="timetable"
        :span-method="objectSpanMethod"
        border
        :header-cell-style="{ background: '#d9e5fd', color: 'black', fontWeight: 800 }"
      ...
      >
        ...
        >
          <template #default="scope">
            <!-- 课程信息展示 -->
            <h4>{{ scope.row[week]?.courseName }}</h4>
  ...
  methods: {
    ...
    fetchCourses() {
      listArrange(loadAllParams).then(response => {
        this.events = response.rows.map(course => ({
          weekday: course.weekday,       // 星期几
    ...
    mergeData() {
      ...
      this.events.forEach(event => {
        const weekKey = this.weeks[event.weekday - 1]
