<template>
  ...
  import { listStudent, getStudent, delStudent, addStudent, updateStudent,getStudentInfo } from "@/api/teach/student";
  ...
  const studentByCodeList = ref([]);
  function getStudentByCode(){
    getStudentInfo().then(response => {
      studentByCodeList.value = response.rows;
